<template>
  <div class="inventory-dashboard-content main-page-content">
      ...
      <div class="content-product" style="height: 75vh;">
        ...
        <div class="content-data" v-if="!loadingFields.stockLoading && !loadingFields.productListLoading">
          <div class="btn-wrapper margin-btn" v-if="!isShowFormAddProductInPurchase" v-can="'inventory_access'">
            ...
            <b-button v-if="stockTransfer.show === false" href="#" size="sm" variant="primary"
              title="Transfer product to new stock" @click="showTransferStock()">
            ...
            <div class="margin-5" v-if="isShowFormAddProductInPurchase && !loadingFields.productListLoading" v-can="'inventory_access'">
              ...
              <b-table :items="items" :fields="fields" stacked="md" show-empty small>
                <template #cell(actions)="row">
                  <b-button size="sm" title="Adjust product select" variant="success"
                    @click="adjustProductAdd(row.item, row.index, $event.target)">
                    <i class="fa fa-edit"></i>
                  </b-button>
                  <b-button size="sm" variant="primary" title="Remove product select from list"
                    @click="showRemoveProductSelect(row.item, row.index, $event.target)" class="mr-1">
    ...
    <b-modal id="modal-create-supplier" ref="supplier-form-modal" size="lg" @hidden="onResetSupplier"
      ...
      :title="$t('title_supplier')" no-close-on-backdrop>
      <b-form enctype="multipart/form-data" @submit.stop.prevent="onSubmitSupplier">
        ...
        <div class="full-content">
          ...
          <b-row class="my-1">
            ...
            <b-col sm="8">
              <b-form-input :id="'input-address'" type="text" v-model="supplier.address" class="input-content">
              </b-form-input>
            </b-col>
          </b-row>
          <b-row class="my-1">
            ...
            <b-col sm="8">
              <b-form-input :id="'input-email'" type="email" v-model="supplier.email" class="input-content">
              </b-form-input>
            </b-col>
          </b-row>
          <b-row class="my-1">
    ...
    <b-modal id="modal-create-warehouse" ref="warehouse-form-modal" size="lg" @hidden="onResetWareHouse"
      :cancel-title="$t('label_cancel_button')" @ok="handleOnSubmitWareHouse" :ok-title="$t('label_save_button')"
      :title="$t('title_new_warehouse')" no-close-on-backdrop>
      <b-form enctype="multipart/form-data" @submit.stop.prevent="onSubmitWareHouse">
        ...
        <div class="full-content">
          <b-row class="my-1">
            <b-col sm="4"><label :for="'input-name'" class="label-input">{{ $t('label_name') }}</label></b-col>
            <b-col sm="8">
              <b-form-input :id="'input-name'" type="text" v-model="warehouse.name" class="input-content">
              </b-form-input>
            </b-col>
          </b-row>
          <b-row class="my-1">
            <b-col sm="4"><label :for="'input-address'" class="label-input">{{$t('label_address')}}</label></b-col>
            <b-col sm="8">
              <b-form-input :id="'input-address'" type="text" v-model="warehouse.address" class="input-content">
              </b-form-input>
            </b-col>
          </b-row>
        </div>
    ...
    <b-modal id="modal-create-existing-product" ref="existing-product-form-modal" size="lg"
      @hidden="onResetExistingProduct" :cancel-title="$t('label_cancel_button')" @ok="handleOnSubmitExistingProduct"
      :ok-title="$t('label_save_button')" :ok-disabled="!product_select.qty" :title="$t('title_add_product')"
          ...
          <div class="display-inline-block full-with">
            <div class="col-md-12 float-right">
              <div class="form-group form-content-detail">
                ...
                <div class="select-content-inline display-inline-block multiple-select"
                  style="width: 68% !important; float: left;">
                  <multiselect v-model="product_select.product" :options="products" track-by="name" label="name"
                    :show-labels="false" :placeholder="$t('label_search_by_product')" @select="selectedProduct"
              ...
              <div class="form-group form-content-detail">
                ...
                <b-form-input class="select-content-inline display-inline-block" v-model="product_select.import_price"
                  :disabled="isDisabledImportPrice === true"></b-form-input>
              </div>
              <div class="form-group form-content-detail">
                <label class="label-with">{{$t('label_sale_price')}} ($)</label>
                <b-form-input class="select-content-inline display-inline-block" v-model="product_select.sale_price"
                  :disabled="true"></b-form-input>
              </div>
              <div class="form-group form-content-detail">
                <label class="label-with">{{$t('label_quantity')}}</label>
                <b-form-input class="select-content-inline display-inline-block" v-model="product_select.qty">
                </b-form-input>
              </div>
            </div>
          </div>
    ...
    <b-modal id="modal-remove-existing-product" ref="remove-existing-product-form-modal" size="lg" cancel-title="No"
      @ok="removeProductAdd(product_select)" :ok-title="$t('label_yes_button')" no-close-on-backdrop>
      <h3 class="center">{{ $t('question_remove_existing_product') }}</h3>
    </b-modal>
    <b-modal id="modal-update-stock" ref="remove-update-stock-modal" size="lg" cancel-title="No"
      @ok="handleOnSubmitPurchase" :ok-title="$t('label_yes_button')" no-close-on-backdrop>
